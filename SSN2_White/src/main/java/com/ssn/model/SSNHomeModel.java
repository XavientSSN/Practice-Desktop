...
public class SSNHomeModel {
    ...
    public void createHiveAlbum(JLabel albumLabel) {
        //albumLabel.setIcon(new ImageIcon(getClass().getResource("/icon/create-album-deactive.png")));
        //String inputAlbumName = JOptionPane.showInputDialog(getHomeForm(), "Please Enter Album Name ", "Create Album", JOptionPane.INFORMATION_MESSAGE);
    ...
    public void openSlideshow(JLabel slideshowLabel) {
    ...
    {
         //System.out.println("playSlideShow"+nextStartIndex);
        ArrayList<ImageIcon> imageList = new ArrayList<ImageIcon>();
       // SSNSlideShowPanel ssnSlideShowPanel = null;
        String[] slideShowSupportedFormats = SSNConstants.SSN_SLIDESHOW_IMAGE_FILE_FORMAT_SUPPORTED;
        List<String> slideShowSupportedFormatList = Arrays.asList(slideShowSupportedFormats);
       
        //System.out.println("imageListFiles:://///////////////////////////"+imageListFiles.size());
        // System.out.println(imageListFiles.get(0).getAbsoluteFile()+"///"+imageListFiles.get(1).getAbsoluteFile()+"///"+imageListFiles.get(2).getAbsoluteFile()+"///"+imageListFiles.get(3).getAbsoluteFile());
        List<File> subImageListFiles = null;
        if (endIndex == imageListFiles.size()) {
            nextStartIndex = 0;
            startIndex =0;
            //System.out.println("startIndex ==0" + startIndex + nextStartIndex + endIndex);
        }
        if (imageListFiles.size() > 5 && endIndex < imageListFiles.size()) {
            ...
            if (rem < 5 && (imageListFiles.size() - endIndex) != rem) {
                ...
                //System.out.println("rem if::"+rem+"endIndex::"+endIndex);
            } else {
                endIndex = imageListFiles.size();
               // System.out.println("rem else::"+rem+"endIndex::"+endIndex);
            }
        ...
        //System.out.println("startIndex ==0 default " + startIndex + nextStartIndex + endIndex);
        ...
        //System.out.println("subImageListFiles:::"+subImageListFiles.size());
...
//            if(imageList.size()==0 && imageListFiles.size() > 5 && imageList.size() < imageListFiles.size()){
//                System.out.println("startIndex+5::::::::"+startIndex);
//                playSlideShow(startIndex+5);
//                break;
//            }
    ...
    public void uploadImage() {
        ...
        try {
            if (ssnDestPath != null && !(ssnDestPath.trim().isEmpty()) && !ssnDestPath.trim().equalsIgnoreCase(SSNHelper.getSsnHiveDirPath())&& !ssnDestPath.trim().equalsIgnoreCase("viewAllAlbums") && !ssnDestPath.trim().equalsIgnoreCase("tagUntaggedMedia")&& !ssnDestPath.trim().equalsIgnoreCase("instagramMedia") && !ssnDestPath.trim().contains(SSNHelper.getFacebookPhotosDirPath())) {
                ...
                SSNMessageDialogBox dialogBox = new SSNMessageDialogBox();
                dialogBox.initDialogBoxUI(SSNDialogChoice.NOTIFICATION_DIALOG.getType(), "Information", "", "Please select an local or valid album");
            }
            ...
            Thread thread = new Thread() {
                ...
                public void run() {
                   ...
                   // lLabel.setIcon(gifImage);
                    ...
                    lLabel.setIcon(null);
                    lLabel.repaint();
                    lLabel.setIcon(new ImageIcon(getClass().getResource("/icon/cloud-normal.png")));
                            ...
                            if (ssnAlbumMedia.getFile_url() != null && !ssnAlbumMedia.getFile_url().isEmpty()
                                ...
                                try {
                                    ...
                                    if (mediaFile.length() > 0) {
                                        ...
                                        long id = SSNDao.findSyncronisedMediaId(ssnAlbumMedia.getOriginal_file_name(), album.getName());
                                        if (id <= 0) {
                                            SSNDao.saveCloudMedia(ssnAlbumMedia.getId(),
                                                    ssnAlbumMedia.getOriginal_file_name(), album.getName());
                                        }
                                    } else {
                                        mediaFile.delete();
                                    }
    ...
    private void uploadImages(Set<String> fileSet, String accessToken) {
        ...
        try {
                ...
                if(fileToUpload.length() > 0){
                   ...
                   // System.out.println("accessToken " + accessToken);
                    ...
                    if(Arrays.asList(SSNConstants.SSN_UPLOAD_FILE_FORMAT_SUPPORTED).contains(fileExtention.toUpperCase())){
                       // System.out.println("mime type " + SSNHelper.getContentType(fileExtention.toUpperCase()));
                        ...
                        //System.out.println("mime type2 " + fileExtention.toUpperCase());
                    }
                        //System.out.println("MIME Type  " +  ContentType.create(SSNHelper.getContentType(fileExtention.toUpperCase())));
                   // System.out.println("file Extension : " + fileExtention);
                    ...
                    if (metadata != null && metadata.getTitle() != null) {
                        builder.addTextBody("meta_title", metadata.getCaption(), ContentType.TEXT_PLAIN);
                         //System.out.println("meta_title"+ metadata.getCaption() );
                    }
                    ...
                    if (metadata != null && metadata.getMediaLocation() != null) {
                        ...
                        //System.out.println("meta_place"+ metadataDb.getEditMediaLocation() );
                    }
                    ...
                    if (metadata != null && metadata.getSsnKeywords() != null) {
                        builder.addTextBody("meta_keywords", metadata.getSsnKeywords(), ContentType.TEXT_PLAIN);
                         //System.out.println("meta_keywords"+ metadata.getSsnKeywords());
                    }
            ...
            if (success) {
                        ...
                        messageDialogBox.initDialogBoxUI(SSNDialogChoice.NOTIFICATION_DIALOG.getType(), "Message", "", "Sync with cloud complete.");
